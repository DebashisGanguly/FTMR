...
public class TestJobQueueTaskScheduler extends TestCase {
	...
	static class FakeJobInProgress extends JobInProgress {
		...
		public Task obtainNewMapTask(final TaskTrackerStatus tts, int clusterSize,
			...
			Task task = new MapTask("", attemptId, 0, "", new BytesWritable(), null, null) {
				@Override
			...
			setRunningMapTasks(getRunningMapTasks()+1);
			return task;
		}
		...
		public Task obtainNewReduceTask(final TaskTrackerStatus tts,
			...
			Task task = new ReduceTask("", attemptId, 0, 10, 0) {
				@Override
				public String toString() {
					return String.format("%s on %s", getTaskID(), tts.getTrackerName());
				}
			...
			taskTrackerManager.update(tts.getTrackerName(), task);
			setRunningReduceTasks(getRunningReduceTasks()+1);
			return task;
		}

		private TaskAttemptID getTaskAttemptID(boolean isMap) {
			JobID jobId = getJobID();
			return new TaskAttemptID(jobId.getJtIdentifier(), jobId.getId(), isMap, ++taskCounter, 0, 0);
		}
	}
