...
public class GridMixRunner {
    ...
    private static final String VARCOMPSEQ = GRID_MIX_DATA
            + "/WebSimulationBlockCompressed";
    private static final String FIXCOMPSEQ = GRID_MIX_DATA
            + "/MonsterQueryBlockCompressed";
    private static final String VARINFLTEXT = GRID_MIX_DATA
            + "/SortUncompressed";
    ...
    private enum GridMixJob {
        ...
        JAVASORT("javaSort") {
            public void addJob(int numReducers, boolean mapoutputCompressed,
                    boolean outputCompressed, Size size, JobControl gridmix) {
                final String prop = String.format("javaSort.%sJobs.inputFiles",
                        size);
                final String indir = getInputDirsFor(prop, size
                        .defaultPath(VARINFLTEXT));
                final String outdir = addTSSuffix("perf-out/sort-out-dir-"
                        + size);
        ...
        WEBDATASCAN("webdataScan") {
            public void addJob(int numReducers, boolean mapoutputCompressed,
                    boolean outputCompressed, Size size, JobControl gridmix) {
                final String prop = String.format(
                        "webdataScan.%sJobs.inputFiles", size);
                final String indir = getInputDirsFor(prop, size
                        .defaultPath(VARCOMPSEQ));
        ...
        COMBINER("combiner") {
            public void addJob(int numReducers, boolean mapoutputCompressed,
                    boolean outputCompressed, Size size, JobControl gridmix) {
                final String prop = String.format("combiner.%sJobs.inputFiles",
                        size);
                final String indir = getInputDirsFor(prop, size
                        .defaultPath(VARCOMPSEQ));
                final String outdir = addTSSuffix("perf-out/combiner-out-dir-"
                        + size);
                ...
                sb.append("-mapoutputCompressed ").append(mapoutputCompressed)
                .append(" ");
        ...
        MONSTERQUERY("monsterQuery") {
            public void addJob(int numReducers, boolean mapoutputCompressed,
                    boolean outputCompressed, Size size, JobControl gridmix) {
                final String prop = String.format(
                        "monsterQuery.%sJobs.inputFiles", size);
                final String indir = getInputDirsFor(prop, size
                ...
                try {
                    ...
                    for (int i = 0; i < iter; i++) {
                        ...
                        String indirfull = (0 == i) ? indir : outdir + "."
                                + (i - 1);
        ...
        WEBDATASORT("webdataSort") {
            public void addJob(int numReducers, boolean mapoutputCompressed,
                    boolean outputCompressed, Size size, JobControl gridmix) {
                final String prop = String.format(
                        "webdataSort.%sJobs.inputFiles", size);
                final String indir = getInputDirsFor(prop, size
                        .defaultPath(VARCOMPSEQ));
                final String outdir = addTSSuffix("perf-out/webdata-sort-out-dir-"
                        + size);
    ...
    private void addJobs(GridMixJob job, Size size) throws IOException {
        ...
        int[] numJobs = getInts(config, prefix + ".numOfJobs", size
                .defaultNumJobs());
        int[] numReduces = getInts(config, prefix + ".numOfReduces", size
                .defaultNumReducers());

        if (numJobs.length != numReduces.length) {
            throw new IOException("Configuration error: " + prefix
                    + ".numOfJobs must match " + prefix + ".numOfReduces");
        }

        int numMapoutputCompressed = config.getInt(prefix
                + ".numOfMapoutputCompressed", 0);
        int numOutputCompressed = config.getInt(prefix
                + ".numOfOutputCompressed", size.defaultNumJobs());
        ...
        for (int i = 0; i < numJobs.length; ++i) {
            for (int j = 0; j < numJobs[i]; ++j) {
                boolean mapoutputComp = select(totalJobs,
                        numMapoutputCompressed, currentIndex);
                boolean outputComp = select(totalJobs, numOutputCompressed,
                        currentIndex);
                job.addJob(numReduces[i], mapoutputComp, outputComp, size,
                        gridmix);
                ++numOfJobs;
                ++currentIndex;
            }
        ...
        try {
            ...
            while (groups.hasNext()) {
                ...
                while (cs.hasNext()) {
                    ...
                    String n = c.getName();
                    long v = c.getCounter();
                    retv.put(mapreduceID + "." + jobName + "." + gn + "." + n,
                            "" + v);
                }
            }
            TaskReport[] maps = jc.getMapTaskReports(JobID.forName(mapreduceID));
            TaskReport[] reduces = jc.getReduceTaskReports(JobID
                    .forName(mapreduceID));
            ...
            if (job.getState() == Job.SUCCESS) {
                ...
                retv.put(mapreduceID + "." + jobName + "." + "jobStatus",
                        "failed");
            } else {
                retv.put(mapreduceID + "." + jobName + "." + "jobStatus",
                        "unknown");
