...
class Merger {  
	...
	throws IOException {
		MergeQueue<K, V> mq = new MergeQueue<K, V>(conf, fs, inputs, deleteInputs, codec, comparator, reporter);
		return mq.merge(keyClass, valueClass, mergeFactor, tmpDir, readsCounter, writesCounter);
	}
	...
	RawKeyValueIterator merge(Configuration conf, FileSystem fs,
			...
			Counters.Counter writesCounter)
	throws IOException {
		MergeQueue<K, V> mq = new MergeQueue<K, V>(conf, fs, segments, comparator, reporter,
				sortSegments);

		return mq.merge(keyClass, valueClass,
				mergeFactor, tmpDir,
				readsCounter, writesCounter);
	}
	...
	RawKeyValueIterator merge(Configuration conf, FileSystem fs,
			...
			Counters.Counter writesCounter)
	throws IOException {
		MergeQueue<K, V> mq = new MergeQueue<K, V>(conf, fs, segments, comparator, reporter, sortSegments, codec);
	...
	public static <K extends Object, V extends Object>
	void writeFile(RawKeyValueIterator records, Writer<K, V> writer, Progressable progressable, Configuration conf) 
	throws IOException {
	...
	public static class Segment<K extends Object, V extends Object> {
		...
		private void init(Counters.Counter readsCounter) throws IOException {
			if (reader == null) {
				...
				LOG.debug("file: " + file + " length: " + segmentLength + " segmentOffset: " + segmentOffset);
			}
		}
			...
			do {
				...
				while (true) {
					...
					for (Segment<K, V> segment : mStream) {
						...
						LOG.debug("Segment " + count + ": start position: " + startPos + " endpos: " + endPos + " startbytes: " + startBytes);
						count++;
				...
				if (numSegments <= factor) {
					...
					LOG.debug("Merger writting to : " + outputFile);
					Writer<K, V> writer = new Writer<K, V>(conf, fs, outputFile, keyClass, valueClass, codec, writesCounter);
