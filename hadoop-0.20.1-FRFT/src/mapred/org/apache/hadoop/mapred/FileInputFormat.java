...
public abstract class FileInputFormat<K, V> implements InputFormat<K, V> {
    ...
    public static void setOutputPaths(JobConf conf, Path... inputPaths) {
        Path path = new Path(conf.getWorkingDirectory(), inputPaths[0]);
        StringBuffer str = new StringBuffer(StringUtils.escapeString(path.toString()));
        for(int i = 1; i < inputPaths.length;i++) {
            str.append(StringUtils.COMMA_STR);
        ...
        conf.set("mapred.output.dir", str.toString());
        conf.set("mapred.output.digests.dir", str.toString() + "/" + SHA_DIR_NAME);
    }
    ...
    public static void addInputPath(JobConf conf, Path path ) {
        ...
        String res = (dirs == null ? dirStr : dirs + StringUtils.COMMA_STR + dirStr);
        conf.set("mapred.input.dir", res);
    }
    ...
    public static Path[] getOutputPaths(JobConf conf) {
        String dirs = conf.get("mapred.output.dir", "");
        String [] list = StringUtils.split(dirs);
        Path[] result = new Path[list.length];
        for (int i = 0; i < list.length; i++) {
            result[i] = new Path(StringUtils.unEscapeString(list[i]));
