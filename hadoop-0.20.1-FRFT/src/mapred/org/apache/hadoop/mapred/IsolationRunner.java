...
public class IsolationRunner {
    ...
    private static class FakeUmbilical implements TaskUmbilicalProtocol {
        ...
        public void requestMap(TaskID taskid) throws IOException {
            LOG.info("Task " + taskid + " request map.");
        }
     ...
     */
    private static void fillInMissingMapOutputs(FileSystem fs, TaskAttemptID taskId, int numMaps, JobConf conf) 
            throws IOException 
            {
        Class<? extends WritableComparable> keyClass = conf.getMapOutputKeyClass().asSubclass(WritableComparable.class);
        Class<? extends Writable> valueClass = conf.getMapOutputValueClass().asSubclass(Writable.class);
    ...
    public static void main(String[] args)
        ...
        if (isMap) {
            ...
            splitFile.close();
            task = new MapTask(jobFilename.toString(), taskId, partition, splitClass, split, "NONE", new String[]{"NONE"});
        } else {
            int numMaps 	= conf.getNumMapTasks();
            int nrOfReplica = conf.getFaultTolerance();

            fillInMissingMapOutputs(local, taskId, numMaps, conf);
            task = new ReduceTask(jobFilename.toString(), taskId, partition, numMaps, nrOfReplica);
        }
        task.setConf(conf);
