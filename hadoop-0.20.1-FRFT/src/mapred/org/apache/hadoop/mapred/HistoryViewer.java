...
class HistoryViewer {
    ...
    public HistoryViewer(String outputDir, Configuration conf, boolean printAll)
        ...
        try {
            ...
            int size = jobFiles.length;
            init(outputDir, conf, printAll, size);
    ...
    public void init(String outputDir, Configuration conf, boolean printAll, int size)
            throws IOException {
        this.conf = conf;
        this.printAll = printAll;
        Path output = new Path(outputDir);
        ...
        try {
            ...
            for(int i=0; i<size; i++) {
                jobLogFile = jobFiles[i].toString();
                String[] jobDetails = 
                        JobInfo.decodeJobHistoryFileName(jobFiles[i].getName()).split("_");
                ...
                DefaultJobHistoryParser.parseJobTasks(jobFiles[i].toString(), job, fs);
                print();
            }
        } catch(Exception e) {
            throw new IOException("Not able to initialize History viewer", e);
