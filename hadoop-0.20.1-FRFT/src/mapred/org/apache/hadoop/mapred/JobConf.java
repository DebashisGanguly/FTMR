...
public class JobConf extends Configuration {
     ...
     * It's the jar that will execute MR job.
     * 
     ...
     * into the configuration under the mapred.jar key
     * 
    ...
    public String[] getLocalDirs() throws IOException {
        if(localDirs == null)
            localDirs = getStrings("mapred.local.dir");
        return localDirs;
    }
    ...
    public boolean getKeepJobFiles() {
        return getBoolean("mapred.keep.job.files", false);
    }
    ...
    public InputFormat getOutputFormatAsInput() {
        return ReflectionUtils.newInstance(getClass("mapred.output.format.class",
                TextInputFormat.class,
                InputFormat.class),
                this);
    ...
    public Class<? extends CompressionCodec> 
    getOutputCompressorClass(Class<? extends CompressionCodec> defaultValue) {
        Class<? extends CompressionCodec> codecClass = defaultValue;
    ...
    public boolean getValidateReduceOutput() { 
        return getBoolean("mapred.reduce.tasks.output.validate", false);
    }

    /** 
     * Should run only f+1 map tasks
     * Defaults to <code>false</code>.
     * 
     * @return
     */
    public boolean getForceKillTaskExecution() { 
    ...
    public void setDeferredExecution(boolean value) { 
        setBoolean("mapred.map.tasks.deferred.execution", value);
    }
    ...
    public boolean getMapFileTamperingExecution() { 
        return getBoolean("mapred.map.tasks.tampering.file.execution", false);
    }

    public boolean getReduceTamperingExecution() { 
        return getBoolean("mapred.reduce.tasks.tampering.digests.execution", false);
    }
    ...
    public boolean getTaskErrors() { 
        return getBoolean("mapred.map.tasks.errors", true);
    }

    public boolean getForceShuffleInDisk() { 
        return getBoolean("mapred.map.tasks.force.shuffle.disk", false);
    }

    Integer distributionThreshold = null;
    public int getDistributionThreshold() { 
        if(distributionThreshold == null) {
            distributionThreshold = getInt("mapred.map.tasks.distribution.threshold", 1);
            if(distributionThreshold > getFaultTolerance())
                distributionThreshold = getFaultTolerance();
        }
    ...
    /**
     * Get configured the number of map tasks for this job.
     * Defaults to <code>1</code>.
     * 
     * @return the number of reduce tasks for this job.
     */
    public int getNumMapTasks() { return getInt("mapred.map.tasks", 3); }

    /**
     * Get number of replica defined in mapred-site.xml
     * @return
     */
    private int faults = 0;
    public int getFaultTolerance() {
        if(faults == 0) {
            faults = getInt("tasktracker.tasks.fault.tolerance", 1);
        }
        return 2*faults+1;
    }
     ...
     * @param n
     */
    public void setFaultTolerance(int n) { setInt("tasktracker.tasks.fault.tolerance", n); }
    ...
    public void  setReduceFinishScript(String rDbgScript) {
        set("mapred.reduce.task.finish.script", rDbgScript);
    }

    public String getReduceFinishScript() {
        return get("mapred.reduce.task.finish.script");
    }
    ...
    public String getQueueName() {
        return queueName == null ? queueName = get("mapred.job.queue.name", DEFAULT_QUEUE_NAME) : queueName;
    }
    ...
    private static String findContainingJar(Class my_class) {
        ...
        try {
            ...
            throw new RuntimeException("Couldn't find " + class_file, e);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        return null;
