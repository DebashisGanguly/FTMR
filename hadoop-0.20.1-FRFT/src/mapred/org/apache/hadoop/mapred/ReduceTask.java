...
class ReduceTask extends Task {
    ...
    public static final Log LOG = LogFactory.getLog(ReduceTask.class.getName());
    private int numMaps;

    private ReduceCopier reduceCopier;

    private CompressionCodec codec;
    private final int TAMPERDIGEST = 3;
    {
    ...
    private final SortedSet<FileStatus> mapOutputFilesOnDisk = new TreeSet<FileStatus>(mapOutputFileComparator);

    /*
     * java.lang.Exception:
        at org.apache.hadoop.mapred.ReduceTask.<init>(ReduceTask.java:164)
        at org.apache.hadoop.mapred.LaunchTaskAction.readFields(LaunchTaskAction.java:62)
        at org.apache.hadoop.mapred.HeartbeatResponse.readFields(HeartbeatResponse.java:137)
        ...
        at org.apache.hadoop.ipc.Client$Connection.receiveResponse(Client.java:510)
    ...
    /*
     * java.lang.Exception:
        at org.apache.hadoop.mapred.ReduceTask.<init>(ReduceTask.java:171)
        at org.apache.hadoop.mapred.TaskInProgress.addRunningTask(TaskInProgress.java:1034)
        at org.apache.hadoop.mapred.TaskInProgress.addRunningTask(TaskInProgress.java:1004)
        at org.apache.hadoop.mapred.TaskInProgress.getTaskToRun(TaskInProgress.java:1000)
        at org.apache.hadoop.mapred.JobInProgress.obtainNewReduceTask(JobInProgress.java:1297)
        at org.apache.hadoop.mapred.JobQueueTaskScheduler.assignTasks(JobQueueTaskScheduler.java:226)
        at org.apache.hadoop.mapred.JobTracker.heartbeat(JobTracker.java:2575)
     ...
     */
    public ReduceTask(String jobFile, TaskAttemptID taskId, int partition, int numMaps, int numReplicas) {
        super(jobFile, taskId, partition);
        this.numMaps = numMaps;
        this.numReplicas 	= numReplicas;

        LOG.debug("ReduceTask created: " + getTaskID());
    }
    ...
    /**
     * Runner to start the ReduceTask as an attempt
     */
    @Override
    ...
    // end of SkippingReduceValuesIterator


    /**
     * Child class starts ReduceTask.
     * This method start running the ReduceTask instance. It's executed by Child.java
     * This method is from ReduceTask.java
     */
    @Override
    @SuppressWarnings("unchecked")
    public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
        ...
        // check if map-output are to be compressed.
        codec = initCodec();

        boolean isLocal = "local".equals(job.get("mapred.job.tracker", "local"));
        if (!isLocal) {
            LOG.info("ReduceCopier created for " + getTaskID());

            reduceCopier = new ReduceCopier(umbilical, job, reporter);
            long start = System.currentTimeMillis();
            // fetch hashes and map output
            ...
            LOG.debug("Shuffle time: " + start + " - " + end);
            reporter.setShuffleTime(start, end);
            setShuffleTime(umbilical, reporter, start, end);
        }

        LOG.info("---------- Copy phase completed for " + getTaskID() + ". All map outputs are copied. ----------");
        copyPhase.complete();                         // copy is already complete

        // -------------------------- Begin of SORT and REDUCE PHASE --------------------------------------
    ...
    @SuppressWarnings("unchecked")
    private <INKEY,INVALUE,OUTKEY,OUTVALUE> void runOldReducer(
            JobConf job,
            TaskUmbilicalProtocol umbilical,
        ...
        String finalName = getOutputName(getPartition(), Util.getReplicaNumber(getTaskID()));
        FileSystem fs = FileSystem.get(job);
        fs.setReplication(new Path(finalName), (short) 1);
        ...
        try {
            ...
            ReduceValuesIterator<INKEY,INVALUE> values = isSkipping() ?
                    new SkippingReduceValuesIterator<INKEY,INVALUE>(rIter, comparator, keyClass, valueClass, job, reporter, umbilical) :
                    new ReduceValuesIterator<INKEY,INVALUE>(rIter, job.getOutputValueGroupingComparator(), keyClass, valueClass, job, reporter);
    ...
    public byte[] generateDigest() {
        String filename = getOutputName(getPartition(), Util.getReplicaNumber(getTaskID()));
        ...
        if(conf.getBoolean("mapred.output.compress", false)) {
            Class<? extends CompressionCodec> codecClass =
                    FileOutputFormat.getOutputCompressorClass(conf, GzipCodec.class);
            // create the named codec
            CompressionCodec codec = ReflectionUtils.newInstance(codecClass, conf);
            extension = codec.getDefaultExtension();

            try {
                if(!dfs.exists(new Path(conf.get("mapred.output.dir") + "/" + filename + extension))) {
                    LOG.warn("Compressed output is enabled but extension file isn't " + extension);
                    extension = "";
                }
            } catch (IOException e) {
                // TODO Auto-generated catch block
        ...
        if(digest != null) {
            ...
            try {
                fout = dfs.create(new Path(conf.get("mapred.output.digests.dir") + "/" + Util.getOutputFileHash(new Path(filename))), (short) 1);
                fout.write(digest, 0, digest.length);
            } catch (IOException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            } finally {
    ...
    /**
     * Start of Reduce Copier
     * This is the part that copies the map outputs.
     * Each Reduce task instance runs a Reduce Copier
     *
     * ./userlogs/attempt_201004301612_0001_r_000001_0_0_r_0/syslog:2010-04-30 16:12:53,223 INFO mapred.ReduceTask:391 ReduceCopier created for job_201004301612_0001
     * ./userlogs/attempt_201004301612_0001_r_000000_0_0_r_0/syslog:2010-04-30 16:12:51,809 INFO mapred.ReduceTask:391 ReduceCopier created for job_201004301612_0001
     *
     * @param <K>
     * @param <V>
     */
    class ReduceCopier<K, V> implements MRConstants {
        ...
        /** Keeps the history of the received digests */
        private List<String> digestsHistory;
        /**
        ...
        private int[] counter = new int[numMaps];

        private int distance[] = new int[numMaps];
        private String selectedTasks[] = new String[numMaps];
        private List<MapOutputLocation> backup =
                Collections.synchronizedList(new ArrayList<ReduceTask.ReduceCopier<K,V>.MapOutputLocation>());
         ...
         */
        private List<MapOutputLocation> retryFetches = Collections.synchronizedList(new ArrayList<MapOutputLocation>());

        /**
         * The set of required map outputs
         */
        private Set <TaskID> copiedMapOutputs = Collections.synchronizedSet(new TreeSet<TaskID>());

        /**
         * The set of obsolete map taskids.
         */
        private Set <TaskAttemptID> obsoleteMapIds = Collections.synchronizedSet(new TreeSet<TaskAttemptID>());
        ...
        private class CopyResult {
            ...
            public boolean getKill() { return size == KILL; }
        ...
        public class MapOutputLocation {
            ...
            public MapOutputLocation(TaskAttemptID taskAttemptId,
                                     String ttHost, URL taskOutput, String hash) {
            ...
            public TaskID getTaskId() {
                return Util.getTaskID(taskAttemptId);
            }
        ...
        class ShuffleRamManager implements RamManager {
            ...
            public ShuffleRamManager(Configuration conf) throws IOException {
                ...
                maxSize = (int)Math.min((Runtime.getRuntime().maxMemory() * maxInMemCopyUse), Integer.MAX_VALUE);
        ...
        private class MapOutputCopier extends Thread {
            ...
            private boolean forceShuffleInDisk;
            ...
            public MapOutputCopier(JobConf job, Reporter reporter) {
                ...
                this.reporter = reporter;
                forceShuffleInDisk = job.getForceShuffleInDisk();
            ...
            public synchronized boolean forceFail() {
                finish(-5);
                return true;
            }
            /**
            ...
            private synchronized void finish(long size) {
                if (currentLocation != null) {
                    LOG.debug(getName() + " finishing " + currentLocation.getTaskAttemptId() + " =" + size);
            ...
            private boolean shouldWait(int limit) {
                String str = "";
                ...
                LOG.debug("[" + str + "] - " + limit + " result: " + res);
                return res;
            }
            ...
            @Override
            public void run() {
                int limit = conf.getDeferredExecution() ? conf.getFaultToleranceLimit()+1 : 1;
                    ...
                    try {
                        ...
                        synchronized (scheduledCopies) {
                            while(true) {
                                ...
                                int mid = loc.getTaskAttemptId().getTaskID().getId();
                                LOG.debug("Backing up " + loc.getTaskAttemptId().getTaskID().toString());
                                backup.add(loc);
                                counter[mid]++;

                                // wait for a limit map tasks finish
                                if(!shouldWait(limit))
                                    break;
                            }
                            ...
                            if(LOG.isDebugEnabled()) {
                                ...
                                for(int i=0; i<backup.size(); i++) {
                                    str += backup.get(i).getTaskAttemptId().toString() + ", ";
                                }
                                LOG.debug("Scheduled result: ["+str+"]");
                            }
                        }

                        LOG.debug("Start fetching");
                        //                        if(conf.getDeferredExecution()) {
                        fetchMapOutputsDeferred(backup);
                        //                        }
                        //                        else {
                        //                            fetchMapOutputsTentative(backup);
                        //                        }
            ...
            private void fetchMapOutputsDeferred(List<MapOutputLocation> backup) throws InterruptedException {
                ...
                if(LOG.isDebugEnabled()) {
                    String str = "";
                    for(int i=0; i<backup.size(); i++)
                        str += backup.get(i).getTaskAttemptId().getTaskID().toString() + ", ";
                    LOG.debug("==> [" + str + "]");
                }

                // create a vector distance with min values
                int rreplica = getTaskID().getTaskID().getReplicaNumber();
                for(int i=0; i<backup.size(); i++) {
                    int mid = backup.get(i).getTaskId().getId();
                    int mreplica = backup.get(i).getTaskId().getReplicaNumber();

                    int res = Math.abs(rreplica - mreplica);
                    if(res < distance[mid]) {
                        distance[mid] = res;
                        selectedTasks[mid] = backup.get(i).getTaskAttemptId().toString();
                    }
                }
                ...
                if(LOG.isDebugEnabled()) {
                    ...
                    for(int i=0; i<distance.length; i++)
                        str3 += distance[i] + ", ";
                    LOG.debug("Distance - [" + str3 + "]");

                    String str4 = "";
                    for(int i=0; i<selectedTasks.length; i++)
                        str4 += selectedTasks[i] + ", ";
                ...
                for(int i=0; i<selectedTasks.length; i++) {
                    for(int j=0; j<backup.size(); j++) {
                        loc = backup.get(j);
                        String tid = loc.getTaskAttemptId().toString();

                        if(!tid.equals(selectedTasks[i]))
                            continue;

                        LOG.debug("Selected: " + tid);
                        int mid = loc.getTaskAttemptId().getTaskID().getId();
                        LOG.debug("Diminuishing counter");
                        counter[mid]--;
                        ...
                        try {
                            ...
                            LOG.warn(reduceTask.getTaskID() + " copy failed: " + loc.getTaskAttemptId() + " from " + loc.getHost());
            ...
            private void fetchMapOutputsTentative(List<MapOutputLocation> backup) throws InterruptedException {
                MapOutputLocation loc;
                long size = -1;
                synchronized (backup) {
                    Iterator<ReduceTask.ReduceCopier<K,V>.MapOutputLocation> iter = backup.iterator();
            ...
            private long copyOutput(MapOutputLocation loc) throws IOException, InterruptedException {
                ...
                synchronized (digestsHistory) {
                    if(!digestsHistory.contains(loc.getHash()))
                        digestsHistory.add(loc.getHash());
                }
                ...
                Path filename = new Path("/"
                        + TaskTracker.getIntermediateOutputDir(reduceId.getJobID().toString(), reduceId.toString())
                        + "/map" + loc.getTaskId().getId() + ".out");
                LOG.debug("Path -> Reduce: " + filename.toString());
                ...
                if (mapOutput == null) {
                    scheduledCopies.add(loc);// put again on the list
                    LOG.warn("Failed to fetch map-output for " + loc.getTaskAttemptId() + " from " + loc.getHost());
                    return -2;
                }
                ...
                synchronized (ReduceTask.this) {
                    ...
                    if (mapOutput.inMemory) {
                        LOG.debug("Map output in memory - " + mapOutput.mapId + " - " + mapOutput.mapId.getId());
                        // Save it in the synchronized list of map-outputs
                        mapOutputsFilesInMemory.add(mapOutput);
                    } else {
                        LOG.debug("Map output in disk");
            ...
            private MapOutput getMapOutput(MapOutputLocation mapOutputLoc, Path filename, int reduce)
                ...
                int MAX_TRIES = 3;
                MapOutput result = null;

                for(int i=0; i<MAX_TRIES; i++) {
                    LOG.debug("Get mapoutput: " + mapOutputLoc.getTaskAttemptId().toString() + " Try " + i);
                    result = getMapOutput(mapOutputLoc, filename, reduce, false);
                    if(result != null)
                        break;
                }

                // If it couldn't be found a majority of digests, abort the task and start again.
                if(result == null)
                    forceFail();
            ...
            private MapOutput getMapOutput(MapOutputLocation mapOutputLoc, Path filename, int reduce, boolean a)
                    throws IOException, InterruptedException {
                // Connect
                URLConnection connection = mapOutputLoc.getOutputLocation().openConnection();
                LOG.debug("URL Class: " + connection.getClass().toString());
                LOG.debug("URL: " + connection.getURL().toString());
                InputStream input = getInputStream(connection, STALLED_COPY_TIMEOUT, DEFAULT_READ_TIMEOUT);
                ...
                LOG.debug("MAPID: " + mapId + " EXPECTEDMAPID: " + expectedMapId);
                ...
                LOG.debug("Fetching " + mapId.toString());
                ...
                if (shuffleInMemory && !forceShuffleInDisk) {
                    LOG.info("Shuffling " + decompressedLength + " bytes (" +
                            compressedLength + " part bytes) " +
                    ...
                    LOG.debug(mapId.toString() + "\nSHUFFLE TO MEM2: " + mapId
                            + "\nMAPOUTPUT LENGTH: " + mapOutput.data.length
                            + "\nMAPOUTPUT SIZE: " + mapOutput.compressedSize
            ...
            private MapOutput shuffleInMemory(MapOutputLocation mapOutputLoc,
                                              ...
                                              InputStream input,
                                              int mapOutputLength,/*decompressed */
                                              int compressedLength)/* compressed */
                    throws IOException, InterruptedException {
                // Reserve ram for the map-output
                boolean createdNow;
                try {
                    createdNow = ramManager.reserve(mapOutputLength, input);
                } catch (InterruptedException e) {
                    LOG.error("RamMager couldn't reserve space");
                    throw new InterruptedException("RamMager couldn't reserve space");
                }
                ...
                LOG.debug("Map Output data: " + Util.bytesToHex(mapOutput.data));
                return mapOutput;
            }
            ...
            private MapOutput shuffleToDisk(MapOutputLocation mapOutputLoc,
                ...
                LOG.debug("Going to save in: " + localFilename.toString());
                MapOutput mapOutput =
                        new MapOutput(mapOutputLoc.getTaskId(), mapOutputLoc.getTaskAttemptId(),
                                conf, localFilename, mapOutputLength);
                ...
                LOG.info("Read " + bytesRead + " bytes from map-output for " +
                        mapOutputLoc.getTaskAttemptId() + " shuffled to " + localFilename.toString());
        ...
        public ReduceCopier(TaskUmbilicalProtocol umbilical, JobConf conf, TaskReporter reporter)
            ...
            this.scheduledCopies = Collections.synchronizedList(new ArrayList<MapOutputLocation>(100));
            ...
            this.digestsHistory = new ArrayList<String>();
            ...
            this.uniqueTasks = new HashSet<String>();
        ...
        public boolean fetchOutputs() throws IOException {
            ...
            List<MapOutputLocation> savedEvents = Collections.synchronizedList(new ArrayList<MapOutputLocation>());
            int[] occurrences = new int[numMaps];

            // ----------------------- Begin of LOOP until we get all required outputs --------------------------
            while (copiedMapOutputs.size() < numMaps && mergeThrowable == null) {
                ...
                if (logNow) {
                    ...
                    LOG.debug("Copied: " + copiedMapOutputs.size() + " / " + numMaps);
                }
                ...
                if (retryFetches.size() > 0) {
                    LOG.info(reduceTask.getTaskID() + ": " + "Got " + retryFetches.size() + " map-outputs from previous failures");
                }
                ...
                synchronized (scheduledCopies) {
                    ...
                    while (hostsItr.hasNext()) {
                        ...
                        // knownOutputsByLoc contains the MapOutputLocation of URL to fetch the map output
                        ...
                        synchronized (knownOutputsByLoc) {
                            ...
                            while (locItr.hasNext()) {
                                ...
                                if(loc.getTaskAttemptId().getTaskID().isSetupOrCleanup()) {
                                    locItr.remove(); continue;
                                }

                                if(uniqueTasks.contains(loc.getTaskAttemptId().toString())) {
                                    if(LOG.isDebugEnabled()) {
                                        LOG.debug("Ignoring " + loc.getTaskAttemptId().toString());
                                        String str = "";
                                        Iterator<String> iter = uniqueTasks.iterator();
                                        while(iter.hasNext())
                                            str += iter.next() + ", ";
                                ...
                                LOG.debug("Adding host: " + host + " in " + loc.getTaskAttemptId().toString());
                                uniqueHosts.add(host);
                                uniqueTasks.add(loc.getTaskAttemptId().toString());
                ...
                while (numInFlight > 0 && mergeThrowable == null) {
                    ...
                    if(cr.getKill()) {
                        ...
                        umbilical.shuffleError(getTaskID(), "Exceeded corrected fetches of map-ouput; bailing-out.");
                    ...
                    if (cr.getSuccess()) {  // a successful copy
                        ...
                        LOG.info("Task " + getTaskID() + ": Failed fetch #" + noFailedFetches + " from " + mapTaskId);
            ...
            // ------------------------ End of LOOP until we get all required outputs ---------------------------------
            ...
            try {
                ...
                LOG.info("getMapsEventsThread threw an InterruptedException: " +
        ...
        private RawKeyValueIterator createKVIterator(JobConf job, FileSystem fs, Reporter reporter)
            ...
            if (mapOutputsFilesInMemory.size() > 0) {
                ...
                if (numMemDiskSegments > 0 &&
                    ...
                    final Path outputPath = mapOutputFile.getInputFileForWrite(mapId, numReplicas, reduceTask.getTaskID(), inMemToDiskBytes);
            ...
            for (Path file : onDisk) {
                ...
                LOG.debug(status.getPermission() + "  " + status.getOwner() + ":" + status.getGroup()
                        + "  " + status.getLen() + ":" + status.getBlockSize() + ":" + status.getReplication()
                        + "  " + status.getPath());
            ...
            synchronized (copyResults) {
                while (copyResults.isEmpty()) {
                    try {
                        ...
                        LOG.error("copy results threw a InterruptedException");
                    }
                }
        ...
        private class InMemFSMergeThread extends Thread {
            ...
            private void doInMemMerge() throws IOException{
                ...
                Path outputPath = mapOutputFile.getInputFileForWrite(mapId, numReplicas, reduceTask.getTaskID(), mergeOutputSize);

                LOG.debug("2 - output path (" + mapId.getId() + " - " + mapId + "): " + outputPath.toString());
                Writer writer =
                        new Writer(conf, rfs, outputPath,
                                conf.getMapOutputKeyClass(),
                                conf.getMapOutputValueClass(),
        ...
        private class GetMapEventsThread extends Thread {
            ...
            // count the occurrences on each map
            int[] replicas = new int[numReplicas];
            int counter = 0; //to force that the requests are processed in order. M0 must execute before M1, even when M1 finished first that M0.
            List<TaskCompletionEvent> backup = new ArrayList<TaskCompletionEvent>();

            public GetMapEventsThread() {
                setName("Thread for polling Map Completion Events");
                setDaemon(true);

                Arrays.fill(replicas, getTaskID().getTaskID().getReplicaNumber());
            }
            ...
            public void run() {
                do {
                    try {
                        ...
                        LOG.warn(reduceTask.getTaskID() +
                                " GetMapEventsThread returning after an interrupted exception");
                        return;
                    }
                    catch (Throwable t) {
                ...
                for (TaskCompletionEvent event : events) {
                    switch (event.getTaskStatus()) {
                        case SUCCEEDED:
                        {
                            ...
                            LOG.debug("Received successful event for " + taskId.toString());
                            ...
                            // the partition that contains the digest, is defined by the id of the reduce tasks.
                            // the number of partitions is equal to the number of reduces.
                            if(LOG.isDebugEnabled()) {
                                String[] digest = event.getDigests();
                                if(digest == null)
                                    LOG.debug("NULL");
                                else {
                                    LOG.debug(Arrays.deepToString(digest));
                                }
                            }

                            String chosen_hash = event.getDigests()[getPartition()];

                            MapOutputLocation mol = new MapOutputLocation(taskId, host, mapOutputLocation, chosen_hash);
                            numNewMaps ++;
                            loc.add(mol);
    ...
    public static String getHex2( byte [] raw ) {
        ...
        for(int i=0; i<raw.length; i++)
            buf.append(Integer.toString(raw[i], 2).toUpperCase());
