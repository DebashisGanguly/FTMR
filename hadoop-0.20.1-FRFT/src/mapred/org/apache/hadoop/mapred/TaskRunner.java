...
abstract class TaskRunner extends Thread {
    ...
    public TaskRunner(TaskTracker.TaskInProgress tip, TaskTracker tracker, JobConf conf, boolean isMap) {
        this.tip = tip;
        this.t = tip.getTask();
     ...
     * 
     * This method is run in the parent process after the child exits.  
     * It should not execute user code, only system code.
     * 
     * It removes the map out if it's a Map task {@link MapTaskRunner}
     * or the reduces input if it's a Reduce task {@link ReduceTaskRunner}
     */
    ...
    /**
     * This methods launches Java classes like Child.java which runs them in JvmTasks
     */
    @Override
                ...
                if ((files != null)) {
                    ...
                    for (int i = 0; i < files.length;i++) {
                        ...
                        p[i] = DistributedCache.getLocalCache(files[i], conf, 
                                new Path(baseDir),
                                fileStatus,
                                false, Long.parseLong(fileTimestamps[i]), new Path(workDir.getAbsolutePath()), false);
                    }
    ...
    public static void setupWorkDir(JobConf conf) 
            throws IOException 
            {
