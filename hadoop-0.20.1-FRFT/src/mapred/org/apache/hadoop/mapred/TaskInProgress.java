...
class TaskInProgress {
    ...
    static final double SPECULATIVE_GAP = 0.2 * 3;
    ...
    private RawSplit rawSplit; // true if it's a map task, false otherwise.
    private int numMaps;
    ...
    private int numForcedKilledTasks = 0;
    ...
    // currently runnings. With speculative execution, it's possible 
    // to have more that one activeTask running at the same time
    ...
    public TaskInProgress(JobID jobid, String jobFile, 
            ...
            JobInProgress job, int partition, int numReplica, 
        ...
        setMaxTaskAttempts();
        init(jobid, partition, numReplica, isSetupOrCleanup);
    }
    ...
    public TaskInProgress(JobID jobid, String jobFile, 
        ...
        this.execStartTime = System.currentTimeMillis();
        setMaxTaskAttempts();
        init(jobid, partition, numReplica, false);
    }

    /**
     * Constructor for ReduceTask, CleanUp or Setup Task for reduce
     */
    public TaskInProgress(JobID jobid, String jobFile, 
            int numMaps,  
            JobTracker jobtracker, JobConf conf,
        ...
        // partition is a number that tells which partition this reduce belongs.
        // For ex., if we've splits A,B,C and D, and only 1 reduce tasks,
        // the splits A,B,C and D will run all in this reduce.
        // For ex., if we've splits A,B,C and D, and 2 reduce tasks,
        // the splits A,B will run on partition 0 (reduce 1) 
        // and C,D will run on partition 1 (reduce 2)
        this.partition = partition;
        this.jobtracker = jobtracker;
        this.job = job;
        this.conf = conf;
    ...
    public TaskInProgress(JobID jobid, String jobFile, 
            int numMaps,  
            JobTracker jobtracker, JobConf conf,
            JobInProgress job, int partition, int numReplica, boolean isSetupOrCleanup) {
        this.jobFile = jobFile;
        this.numMaps = numMaps;
        // partition is a number that tells which partition this reduce belongs.
        // For ex., if we've splits A,B,C and D, and only 1 reduce tasks,
     ...
     */
    private void init(JobID jobId, int partition, int numReplica, boolean isSetupOrCleanup) {
        this.startTime = System.currentTimeMillis();

        if(partition >= 0) {
            // on reduce side, the partition is the nr of split,
            // the splitNr is the nr of reduce task
            // and nrR is the replica number 
            this.id = new TaskID(jobId, isMapTask(), partition, numReplica, isSetupOrCleanup); // taskid for map tasks
        }
        else
            LOG.warn("Couldn't create the task. State is imponderable"); // TODO:
     ...
     */
    public long getExecStartTime() { return execStartTime; }

    /**
     * Set the exec start time
     */
    public void setExecStartTime(long startTime) { execStartTime = startTime; }
    ...
    public String printInfo() {
        return this.id + " activeTasks of: " + (activeTasks != null ? activeTasks.entrySet() + " - " + activeTasks.isEmpty() 
                + " - " + activeTasks.size() 
                : " NONE ") + ": SuccessfulTaskid: " 
                + (successfulTaskId != null ? successfulTaskId.toString() : "NONE");
    }
    ...
    public int numForcedKilledTasks() {
        return numForcedKilledTasks;
    }

    public synchronized boolean isForcedKilledTask() {
        return (numForcedKilledTasks > 0);
    }
    /**
    ...
    public boolean shouldClose(TaskAttemptID taskid) {
        ...
        if ((ts != null) &&
                ...
                ((this.failed) ||
                        ((job.getStatus().getRunState() != JobStatus.RUNNING && // in these states (RUNNING and PREP)job already finished
                        (job.getStatus().getRunState() != JobStatus.PREP))))) 
        {
            tasksReportedClosed.add(taskid);
            close = true;
        } else if (isComplete() && // only reduce tasks
                !(isMapTask() 
                        && !jobSetup 
                        && !jobCleanup 
                        && isComplete(taskid))
                        && !tasksReportedClosed.contains(taskid)) 
        {
            tasksReportedClosed.add(taskid);
            close = true; 
        } else if (isCommitPending(taskid) 
                && !shouldCommit(taskid) 
                && !tasksReportedClosed.contains(taskid)) 
        {
            tasksReportedClosed.add(taskid);
            ...
            //PORQUE é que  task não estã no tasksToKill???
    ...
    public boolean shouldCommit(TaskAttemptID taskid) {
        boolean res = taskToCommit.equals(taskid);

        LOG.debug(taskid + " should commit? Complete: " + isComplete() + " commit pending: " 
                + isCommitPending(taskid) + " taskToCommit: "+ res 
                + " Result: " + (!isComplete() && isCommitPending(taskid) && res));

        return !isComplete() && isCommitPending(taskid) && res;
    }
    ...
    synchronized TaskReport generateSingleReport() {
        ...
        long shuffleFinishTime = 0, shuffleStartTime = 0, sortFinishTime = 0;
        for(Iterator<TaskAttemptID> iter = taskStatuses.keySet().iterator(); iter.hasNext(); ) {
            TaskAttemptID tid = iter.next();
            if(!tid.isMap()) {
                // get the lowest and the highest value

                ReduceTaskStatus st = (ReduceTaskStatus)taskStatuses.get(tid);
                if(st.getShuffleFinishTime() > shuffleFinishTime)
                    shuffleFinishTime = st.getShuffleFinishTime();

                if(st.getShuffleStartTime() > shuffleStartTime)
                    shuffleStartTime = st.getShuffleStartTime();

                if(st.getSortFinishTime() > sortFinishTime)
                    sortFinishTime = st.getSortFinishTime();
            }
        }

        String trackerName = "Not Defined";
        if(tasks.size() > 0)
            trackerName = machineWhereTaskRan(tasks.first());

        TaskReport report = new TaskReport(getTIPId(), 
                (float)progress, state,
                diagnostics.toArray(new String[diagnostics.size()]),
                currentStatus,
                execStartTime, 
                sortFinishTime, 
                shuffleFinishTime,
                execFinishTime, counters, trackerName);
    ...
    synchronized boolean updateStatus(TaskStatus status) {
        ...
        if (oldStatus != null) {
            ...
            LOG.debug("TESTE:" + taskid + " : " +  oldState + " -> " +  newState);
            ...
            // We have seen out of order status messages  moving tasks from complete
        ...
        if (!isCleanupAttempt(taskid)) {
            ...
            if(st != null) {
                st.statusUpdate(status.getRunState(),
                        status.getProgress(), status.getStateString(), status.getPhase(), status.getFinishTime());
            }
        }
    ...
    public void incompleteSubTask(TaskAttemptID taskid, JobStatus jobStatus) {
        ...
        LOG.debug("IncompleteSubTask: " + taskid);
        ...
        if (taskid != null && tasks.contains(taskid)) {
            if (taskState == TaskStatus.State.FAILED) {
                numTaskFailures++;
                LOG.debug("Hostname: " + trackerHostName);
    ...
    private void completedTask(TaskAttemptID taskId, TaskStatus.State finalTaskState) {
        LOG.debug(taskId + " (" + getTIPId() + ") has completed " + finalTaskState);

        TaskStatus status = taskStatuses.get(taskId);
        status.setRunState(finalTaskState);
        String trash = activeTasks.remove(taskId);

        if(trash != null)
            LOG.debug(trash + " removed with taskId " + taskId.toString());
    }
    ...
    public TaskStatus[] getTaskStatuses() {
        return taskStatuses.values().toArray(new TaskStatus[0]);
    }
     ...
     */
    public Task getTaskToRun(String taskTracker, String host) 
            throws IOException {
        if (0 == execStartTime){
        ...
        if (nextTaskId < (MAX_TASK_EXECS + maxTaskAttempts + numKilledTasks)) {
            ...
            int attemptId = job.getNumRestarts() * NUM_ATTEMPTS_PER_RESTART + nextTaskId;// nr de tentativa
            int a=0;
            if(attemptId > 0)
                a=1;
            //id -> task_201007091216_0001_r_000000_0_r
            taskid = new TaskAttemptID(id, attemptId);//setup task
        ...
        return addRunningTask(taskid, taskTracker, host);
    }

    /**
     * 
     * @param taskid TaskAttemptID attempt_(...)
     * @param taskTracker taskTracker name
     * @return
     */
    public Task addRunningTask(TaskAttemptID taskid, String taskTracker, String host) {
        return addRunningTask(taskid, taskTracker, host, false);
    }
     ...
     * jobtracker restarts.
     */
    public Task addRunningTask(TaskAttemptID taskid, String taskTracker, String host, boolean taskCleanup) {
        // create the task
        Task t = null;
        ...
        if (isMapTask()) {
            ...
            // create an attempt map task
            t = new MapTask(jobFile, taskid, partition, splitClass, split, rawSplit.getFileName(), rawSplit.getLocations());
            LOG.debug(t.getTaskID().toString() + " - Raw split: " + rawSplit.getFileName() + " - " + Arrays.deepToString(rawSplit.getLocations()));
        } else { // Create an attempt reduce task
            t  = new ReduceTask(jobFile, taskid, getIdWithinJob(), numMaps, conf.getFaultTolerance());
        }
        ...
        if (taskCleanup) {
            ...
            if(ts == null) 
                LOG.error(taskid + " couldn't get TaskStatus");
            else
                t.setState(ts.getRunState());
    ...
    String machineWhereTaskRan(TaskAttemptID taskid) {
        if(taskStatuses.get(taskid) != null)
    ...
    void completeAllTasks() {
        Iterator<TaskAttemptID> iter = activeTasks.keySet().iterator();
