...
class MapTask extends Task {
    ...
    private final static int TAMPERDIGEST = 1;
    private final static int TAMPERDATA = 2;

    private String spillOutputFile;
    ...
    public MapTask(String jobFile, TaskAttemptID taskId, int partition, String splitClass, BytesWritable split, String fileName, String[] locations) {
    ...
    public void setLocations(String locations) {
        this.locations = locations.trim().split(",");
    }
    ...
    public String getSpillOutputFile() {
        return spillOutputFile;
    }
    ...
    public void write(DataOutput out)
        ...
        if (isMapOrReduce()) {
            ...
            out.writeUTF(getLocationsAsString());
        }
    }
    ...
    private class NewOutputCollector<K,V>
        ...
        @Override
        public void close(TaskAttemptContext contex)
                throws IOException,InterruptedException {
            try {
                collector.flush();
    ...
    void runNewMapper(final JobConf job,
                      ...
                      TaskReporter reporter)
            throws IOException, ClassNotFoundException, InterruptedException {
        LOG.debug("Running new mapper");

        // make a task context so we can get the classes
        org.apache.hadoop.mapreduce.TaskAttemptContext taskContext =
                new org.apache.hadoop.mapreduce.TaskAttemptContext(job, getTaskID());

        List<org.apache.hadoop.mapreduce.Mapper<INKEY,INVALUE,OUTKEY,OUTVALUE>> mapperList =
                new ArrayList<org.apache.hadoop.mapreduce.Mapper<INKEY,INVALUE,OUTKEY,OUTVALUE>>();
        ...
        org.apache.hadoop.mapreduce.RecordReader<INKEY,INVALUE> input =
                new NewTrackingRecordReader<INKEY,INVALUE> (inputFormat.createRecordReader(split, taskContext), reporter);
    ...
    class MapOutputBuffer<K extends Object, V extends Object>
        ...
        public MapOutputBuffer(TaskUmbilicalProtocol umbilical, JobConf job, TaskReporter reporter)
            ...
            LOG.debug("keyClass: " + keyClass + " - valClass: " + valClass);
            ...
            LOG.debug("keySerializer: " + keySerializer.getClass() + " - valSerializer: " + valSerializer.getClass());
            // counters
            ...
            try {
                ...
                throw (IOException)new IOException("Spill thread failed to initialize").initCause(sortSpillException);
            } finally {
                spillLock.unlock();
            }
         ...
         * @param K key
         * @param V value
         * @param partition partition chosen (Example, done in HashPartitioner
         */
        ...
        private void sortAndSpill()
            ...
            long size = (bufend >= bufstart ?
                    bufend - bufstart : (bufvoid - bufend) + bufstart) + partitions * APPROX_HEADER_LENGTH;

            FSDataOutputStream out = null;
            try {
                // create spill file
                final SpillRecord spillRec  = new SpillRecord(partitions);
                final Path filename 		= mapOutputFile.getSpillFileForWrite(getTaskID(), numSpills, size);

                spillOutputFile = "" + filename.toUri();
                ...
                String[] hashList = new String[partitions];
                for (int parts = 0; parts < partitions; ++parts) {
                    ...
                    try {
                        ...
                        if (combinerRunner == null) {
                            ...
                            while (spindex < endPosition && kvindices[kvoffsets[spindex % kvoffsets.length] + PARTITION] == parts) {
                                final int kvoff = kvoffsets[spindex % kvoffsets.length];
                                getVBytesForOffset(kvoff, value);
                                key.reset(kvbuffer, kvindices[kvoff + KEYSTART], (kvindices[kvoff + VALSTART] - kvindices[kvoff + KEYSTART]));
                                writer.append(key, value);
                                ++spindex;
                                //									hash.generateHash(md, key, value);
                            }

                            //								hashList[parts] = hash.hashIt(md);
                        ...
                        rec.compressOutput = writer.getCompressOutput();
                ...
                LOG.debug("Exception at the task " + getTaskID() + " - " + ioe.getMessage());
                umbilical.requestMap(getTaskID());
                throw ioe;
            }
            catch(InterruptedException ie) {
                LOG.debug("Exception at the task " + getTaskID() + " - " + ie.getMessage()
                        + ". It's goint to request more map tasks");
                umbilical.requestMap(getTaskID());
                throw ie;
            }
            finally {
...
//                            ShaAbstractHash.convertHashToString(hash);
        ...
        /**
         * Send digests to the job tracker
         * @param digest
         * @throws IOException
         */
            ...
            try {
                ...
                for (int parts = 0; parts < partitions; ++parts) {
                    ...
                    try {
                        ...
                        LOG.debug("2 - Spill idx " + parts + " - SegmentStart: " + segmentStart
                                + " Part length: " + rec.partLength
                                + " Raw length: " + rec.rawLength
                                + " CompressOutput: " + rec.compressOutput
                                + " Compression type: " + rec.compressType);
        ...
        private void mergeParts() throws IOException, InterruptedException, ClassNotFoundException {
            ...
            if (numSpills == 1) { //the spill is the final output
                Path finalOutputFile = new Path(filename[0].getParent(), MapOutputFile.getFileOut());

                rfs.rename(filename[0], finalOutputFile);

                // set permission
                rfs.setPermission(finalOutputFile, FsPermission.createImmutable((short) 0644));
                ...
                if (indexCacheList.size() == 0) {
                    ...
                    // create index files
                    Path finalIndexFile = new Path(filename[0].getParent(), MapOutputFile.getFileOutIndex());
                    rfs.copyFromLocalFile(spillIndexFile, finalIndexFile);
                    rfs.setPermission(finalIndexFile, FsPermission.createImmutable((short) 0644));
                    rfs.delete(mapOutputFile.getSpillIndexFile(mapId, 0), true);
                } else {
                ...
                if(umbilical.shouldTamper(getTaskID(), TAMPERDATA)) {
                    ...
                    int replicatedTasks = MajorityVoting.getNrReplicatedTasks(conf.getNumMapTasks(), conf.getFaultTolerance());
                    BFTInjector.tamperingData(rfs, finalOutputFile, replicatedTasks);
                }
            ...
            LOG.debug("finalOutFileSize: " + finalOutFileSize + " finalIndexFileSize: " + finalIndexFileSize
                    + " finalOutputFile: " + finalOutputFile);

            Path finalIndexFile = mapOutputFile.getOutputIndexFileForWrite(mapId, finalIndexFileSize);
            ...
            if (numSpills == 0) {
                ...
                try {
                    for (int parts = 0; parts < partitions; parts++) {
                        ...
                        LOG.debug("3 - Spill idx " + parts + " - SegmentStart: " + segmentStart
                                + " Part length: " + rec.partLength
                                + " Raw length: " + rec.rawLength
                                + " CompressOutput: " + rec.compressOutput
                                + " Compression type: " + rec.compressType
            ...
            // Writing map output
            {
                ...
                for (int parts = 0; parts < partitions; parts++) {
                    ...
                    LOG.debug("4 - Spill idx " + parts + " - SegmentStart: " + segmentStart
                            + " Part length: " + rec.partLength
                            + " Raw length: " + rec.rawLength
                            + " CompressOutput: " + rec.compressOutput
                            + " Compression type: " + rec.compressType);
            ...
            // send digest to the job tracker
            sendDigest(hash);
        }
    } // MapOutputBuffer
