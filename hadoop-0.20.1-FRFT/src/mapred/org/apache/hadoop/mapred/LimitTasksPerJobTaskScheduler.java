...
class LimitTasksPerJobTaskScheduler extends JobQueueTaskScheduler {
	...
	throws IOException {
		...
		/*
		 * When step == 0, this loop starts as many map tasks it can write
		 * maxTasksPerJob
		 * When step == 1, this loop starts as many reduce tasks it can write
		 * maxTasksPerJob
			...
			for (int step = beginAtStep; step <= 3; ++step) {
				...
				synchronized (jobQueue) {
					for (JobInProgress job : jobQueue) {
						...
						if (step == 0 || step == 2) {
							task = job.obtainNewMapTask(taskTracker, numTaskTrackers, taskTrackerManager.getNumberOfUniqueHosts());
						}
						else {
							task = job.obtainNewReduceTask(taskTracker, numTaskTrackers, taskTrackerManager.getNumberOfUniqueHosts());
						}
