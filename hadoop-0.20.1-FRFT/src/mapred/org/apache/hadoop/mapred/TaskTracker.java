...
import static org.apache.hadoop.mapred.Util.getTaskAttemptID;
...
implements MRConstants, TaskUmbilicalProtocol, Runnable {
	...
	public static final String MR_CLIENTTRACE_FORMAT =
			"src: %s" +     // src IP
					...
					", op: %s" +    // operation
					", cliID: %s";  // task id
	public static final Log ClientTraceLog = LogFactory.getLog(TaskTracker.class.getName() + ".clienttrace");
	...
	// Map<taskId, tip> to be used on taskToclose
	private Map<TaskAttemptID, TaskInProgress> tasks = new HashMap<TaskAttemptID, TaskInProgress>();

	// Collects all digests and then report to JT when a task end.
	// For map task, it's and task ID for a set of digests.
	// For reduce tasks only generate 1 digest
	private Map<TaskID, String[]> digestCollection = new HashMap<TaskID, String[]>();

	private List<TaskAttemptID> tentativeExecutionTasks = Collections.synchronizedList(new ArrayList<TaskAttemptID>());

	/**
	 * Map from taskId -> TaskInProgress.
	 */
	Map<TaskAttemptID, TaskInProgress> runningTasks = null;// map that contains running tasks

	Map<JobID, RunningJob> runningJobs = null;
	volatile int mapTotal = 0; // counter for the number of map tasks
	volatile int reduceTotal = 0; // counter for the number of reduce tasks
	 ...
	 */
	private List<TaskAttemptID> commitResponses = Collections.synchronizedList(new ArrayList<TaskAttemptID>());
	//	private List<String> commitResponses = Collections.synchronizedList(new ArrayList<String>());
	...
	private Thread taskCleanupThread = 
			new Thread(new Runnable() 
			{
				public void run() {
					while (true) {
						try {
							final TaskTrackerAction action = tasksToCleanup.take();
							...
							if (action instanceof KillJobAction) {
								...
								TaskInProgress tip;
								final KillTaskAction killAction = (KillTaskAction) action; 
	...
	private RunningJob addTaskToJob(JobID jobId, TaskInProgress tip) {
		synchronized (runningJobs) {
			...
			synchronized (rJob) {
				rJob.tasksSet.add(tip);
				LOG.debug("Added " + tip.getTask().getTaskID().toString() + " to " + rJob.getJobID());
			}
	...
	private void removeTaskFromJob(JobID jobId, TaskInProgress tip) {
		synchronized (runningJobs) {
			...
			if (rjob == null) {
				...
				synchronized (rjob) {
					boolean result = rjob.tasksSet.remove(tip);
					LOG.info("Removed " + tip.getTask().getTaskID().toString() + " from Running job" + jobId + " - " + result);
				}
			}
		}
	}
	...
	static String getIntermediateOutputDir(String jobid, String taskid) {
		return getLocalTaskDir(jobid, taskid) + Path.SEPARATOR + TaskTracker.OUTPUT ; 
	}
	...
	synchronized void initialize() throws IOException {
		...
		mapLauncher.setName("MapLauncher");
		reduceLauncher.setName("ReduceLauncher");
		mapLauncher.start();
	...
	/**
	 * Thread that tells the reduce which map output it will fetch 
	 */
	...
	/**
	 * Class that tells what map events are done.
	 */
	private class FetchStatus {
		...
		private List<TaskCompletionEvent> allCompletedMapEvents;
		...
		public FetchStatus(JobID jobId, int numMaps) {
			...
			this.jobId = jobId;
			this.allCompletedMapEvents = new ArrayList<TaskCompletionEvent>(numMaps);
		}
		...
		public void reset() {
			...
			synchronized (fromEventId) {
				synchronized (allCompletedMapEvents) {
					fromEventId.set(0); // set the new index for TCE
					allCompletedMapEvents.clear();
				}
			}
		}

		private TaskCompletionEvent[] getMapEvents(int fromId, int max) {
			TaskCompletionEvent[] mapEvents = TaskCompletionEvent.EMPTY_ARRAY;
			boolean notifyFetcher = false;

			synchronized (allCompletedMapEvents) {
				if (allCompletedMapEvents.size() > fromId) {
					int actualMax = Math.min(max, (allCompletedMapEvents.size() - fromId));
					List <TaskCompletionEvent> eventSublist = allCompletedMapEvents.subList(fromId, actualMax + fromId);
		...
		/**
		 * Fetch the map tasks that have completed successfully
		 * @param currTime
		 * @return
		 * @throws IOException
		 */
		public boolean fetchMapCompletionEvents(long currTime)
			...
			synchronized (fromEventId) {
				...
				List <TaskCompletionEvent> recentMapEvents = queryJobTracker(fromEventId, jobId, jobClient);

				synchronized (allCompletedMapEvents) {
					allCompletedMapEvents.addAll(recentMapEvents);
				}
	...
	private void localizeJob(TaskInProgress tip) throws IOException {
		...
		Path localJobFile = lDirAlloc.getLocalPathForWrite(getLocalJobDir(jobId.toString())
				+ Path.SEPARATOR + "job.xml", jobFileSize, fConf);

		// localize job.xml
		LOG.info("Localize Job: LocalJobFile: " + localJobFile + " - JobFile: " + jobFile);
		...
		synchronized (rjob)  {
			if (!rjob.localized) {
				...
				Path workDir = lDirAlloc.getLocalPathForWrite((getLocalJobDir(jobId.toString()) + Path.SEPARATOR + "work"), fConf);
	...
	public TaskTracker(JobConf conf) 
		...
		LOG.debug("Tasktracker running at " + httpBindAddress + ":" + httpPort);

		// Initialize tasktracker server
		...
		server.addInternalServlet("infoOutput", "/infoOutput", InfoOutputServlet.class);
		server.addInternalServlet("taskLog", "/tasklog", TaskLogServlet.class);
		//server.addInternalServlet("hashoutput", "/hashoutput", HashOutputServlet.class);
	 ...
	 * @throws IOException
	 */  
	private List<TaskCompletionEvent> queryJobTracker(IntWritable fromEventId, JobID jobId, InterTrackerProtocol jobClient)
			throws IOException {

		LOG.debug("Requesting to JobID " + jobId.toString() + " from eventId " + fromEventId);
		TaskCompletionEvent t[] = jobClient.getTaskCompletionEvents(jobId, fromEventId.get(), probe_sample_size);

		int count = 0;
		//we are interested in map task completion events only. So store only those
		List <TaskCompletionEvent> recentMapEvents = new ArrayList<TaskCompletionEvent>();
		for (int i = 0; i < t.length; i++) {
			if (t[i] != null && t[i].getTaskAttemptId().getTaskID().isMap()) {
				LOG.debug("Fetch completion event of " + t[i].getTaskAttemptId().toString());
				recentMapEvents.add(t[i]);
				count++;
			}
		}

		fromEventId.set(fromEventId.get() + count);
		...
		while (running && !shuttingDown) {
			try {
				...
				if(justInited) {
					...
					LOG.debug(" SystemFS: " + systemFS.getName() + "; systemDirectory = " + dir);
				}
				...
				if (actions != null) {
					...
					for(TaskTrackerAction action: actions) {
						if (action instanceof LaunchTaskAction) {
							LOG.info("LaunchTaskAction: " + ((LaunchTaskAction) action).getTask() + " from JT heartbeat received");
							addToTaskQueue((LaunchTaskAction)action);
						}
						else if (action instanceof CommitTaskAction) {
							CommitTaskAction commitAction = (CommitTaskAction)action;

							LOG.debug("Commit action received: " + commitAction.getTaskID().toString());

							String t = "";
							for(TaskAttemptID tid : commitResponses) {
								t += tid.toString() + ", ";
							}
							LOG.debug("Commit Responses: " + t);
							LOG.debug("Commit Action: " + commitAction.getTaskID().toString());

							if (!commitResponses.contains(commitAction.getTaskID())) {
								LOG.debug("Received commit task action for " + commitAction.getTaskID());
								commitResponses.add(commitAction.getTaskID());
							}
							//                        }
							//                        else if (action instanceof NotificationTaskAction) {
							//                            TaskAttemptID tid = ((NotificationTaskAction) action).getForcedTaskKilled();
							//                            if(tid != null) {
							//                                synchronized (tasksForcedKilled) {
							//                                    tasksForcedKilled.add(tid);
							//                                }
							//                            }
						} else {
	...
	private HeartbeatResponse transmitHeartBeat(long now) 
		...
		if (ttstatus == null) {
			synchronized (this) {
				ttstatus = new TaskTrackerStatus(taskTrackerName, localHostname, 
						httpPort, 
				...
				synchronized (tentativeExecutionTasks) {
					if(tentativeExecutionTasks.size() > 0) {
						List<TaskID> clone = new ArrayList<TaskID>();
						for (TaskAttemptID tid : tentativeExecutionTasks) {
							LOG.debug("Re-execute again: " + tid.getTaskID().toString());
							clone.add(tid.getTaskID());
						}

						ttstatus.setTasksToRetry(clone);
						tentativeExecutionTasks.clear();
					}
				}
			}
		} else {
		...
		synchronized (this) {
			askForNewTask = (ttstatus.countMapTasks() < maxCurrentMapTasks || 
					ttstatus.countReduceTasks() < maxCurrentReduceTasks) &&
					acceptNewTasks; 
		...
		if (askForNewTask) {
			...
			ttstatus.getResourceStatus().setAvailableSpace(freeDiskSpace);
			ttstatus.getResourceStatus().setTotalVirtualMemory(totVmem);
			ttstatus.getResourceStatus().setTotalPhysicalMemory(totPmem);
			ttstatus.getResourceStatus().setMapSlotMemorySizeOnTT(mapSlotMemorySizeOnTT);
			ttstatus.getResourceStatus().setReduceSlotMemorySizeOnTT(reduceSlotSizeMemoryOnTT);
		}

		if(LOG.isDebugEnabled())
			LOG.debug(ttstatus.printValues());
		//
		// Xmit the heartbeat
		//
		// hearbeat response from task tracker to job tracker.
		HeartbeatResponse heartbeatResponse = jobClient.heartbeat(ttstatus, 
				justStarted,
				justInited,
				askForNewTask, 
				heartbeatResponseId);
		...
		synchronized (this) {
			for (TaskStatus taskStatus : ttstatus.getTaskReports()) {
				if (taskStatus.getRunState() != TaskStatus.State.RUNNING &&
					...
					// task is completed, let's reduce the counter
					...
					if (tip != null) {
						LOG.debug(taskStatus.getTaskID().toString() + " was removed from running tasks. " 
								+ " Total map: " + mapTotal + " Total reduce: " + reduceTotal);
					} else {
						LOG.warn(taskStatus.getTaskID().toString() + " should be removed"
								+ " Total map: " + mapTotal + " Total reduce: " + reduceTotal);
					}
				}
			}
			...
			for (TaskInProgress tip: runningTasks.values()) {
				tip.getStatus().clearStatus();// clear diagnostic info
			}
		}
	 ...
	 * Kill any tasks that have not reported progress in the last X seconds.
	 */
	private synchronized void markUnresponsiveTasks() 
			throws IOException {
		long now = System.currentTimeMillis();
		for (TaskInProgress tip: runningTasks.values()) {
	 ...
	 * @throws IOException
	 */
	private synchronized void purgeJob(KillJobAction action) 
			throws IOException {
		...
		if (rjob == null) {
			...
			synchronized (rjob) {            
				...
				// Delete the job directory for this task if the job is done/failed
				boolean keep = rjob.jobConf.getBoolean("mapred.keep.job.files", false);
				if (!keep || rjob.keepJobFiles){
					directoryCleanupThread.addToQueue(fConf, getLocalFiles(fConf, getLocalJobDir(rjob.getJobID().toString())));
				}
				// Remove this job 
				rjob.tasksSet.clear();
			}
	...
	private void purgeTask(TaskInProgress tip, boolean wasFailure) 
		...
		if (tip != null) {
			...
			if (tip.getTask().isMapTask()) {
				indexCache.removeMap(getTaskAttemptID(tip.getTask()).toString());  
			}
		}
	}
	...
	long tryToGetOutputSize(TaskAttemptID taskId, JobConf conf) {
		try {
			...
			if (!tip.getTask().isMapTask() || tip.getRunState() != TaskStatus.State.SUCCEEDED || taskId.getTaskID().isSetupOrCleanup()) {
				return -1;
			}
	...
	private TaskLauncher mapLauncher; // list of maps to launch
	private TaskLauncher reduceLauncher; // list of reduce to launch
	...
	private void addToTaskQueue(LaunchTaskAction action) {
		if (action.getTask().isMapTask()) {
			LOG.debug("Added LaunchTaskAction to mapLauncher for map: " + action.getTask().getTaskID().toString() + " at " + localHostname);
			mapLauncher.addToTaskQueue(action);
		} else {
			LOG.debug("Added LaunchTaskAction to reduceLauncher");
	...
	private class TaskLauncher extends Thread {
		...
		/**
		 * Register task and add it to tasksToLaunch
		 * @param action
		 */
		private void addToTaskQueue(LaunchTaskAction action) {
			synchronized (tasksToLaunch) {
				...
				LOG.debug("Added " + tip.getTask().toString() + " to tasksToLaunch");
		...
		public void addFreeSlot() {
			synchronized (numFreeSlots) {
				...
				numFreeSlots.notifyAll(); // notify that a free slot is available.
			}
		}

		/**
		 * This is where a TIP is launched to execute MR
		 */
		public void run() {
				...
				try {
					...
					synchronized (tasksToLaunch) {
						while (tasksToLaunch.isEmpty()) {
							tasksToLaunch.wait();// waiting for the tasksToLaunch.notifyAll();
						}

						//get the TIP
						tip = tasksToLaunch.remove(0);
						LOG.info("New task received. Trying to launch : " + tip.getTask().getTaskID());
					}
					...
					if(tip.getTask().isMapTask())
						jobClient.notifyLaunchTasks(tip.getTask().getJobID(), tip.getTask().getTaskID());
	...
	/**
	 * Register task
	 * TaskTracker register the task to launch in a new JVM
	 * 
	 * @param action
	 * @param launcher
	 * @return
	...
	void addToMemoryManager(TaskAttemptID attemptId, boolean isMap, JobConf conf, String pidFile) {
	...
	class TaskInProgress {
		...
		volatile boolean forcedKilled = false;
		 ...
		 */
		 ...
		 /**
		  * It's creates a TIP on the TT side that represents the MR that will run.
		  * 
		  * @param task attempt_201003071655_0002_m_000002_0
		  * @param conf
		  * @param launcher
		  */
		 ...
		 private void localizeTask(Task task) 
				 throws IOException {
			 // create tmp dirs to temp files  
			 Path localTaskDir = 
			 ...
			 if(!localFs.exists(localTaskDir)) {
				 LOG.debug(localTaskDir.toString() + " doesn't exist. Will create one.");
				 if (!localFs.mkdirs(localTaskDir)) {
					 throw new IOException("Mkdirs failed to create " + localTaskDir.toString());
				 }
			 }

			 // create symlink for ../work if it already doesn't exist
			 ...
			 if (fConf.get("slave.host.name") != null) 
				 localJobConf.set("slave.host.name", fConf.get("slave.host.name"));
		 ...
		 public synchronized void setJobConf(JobConf lconf){
			 ...
			 keepFailedTaskFiles = localJobConf.getKeepFailedTaskFiles();
			 taskTimeout = localJobConf.getLong("mapred.task.timeout", 10 * 60 * 1000);
		 }
		  ...
		  * Will run MR task
		  */
		 public synchronized void launchTask() 
				 throws IOException {
					 ...
					 this.taskStatus.getRunState() == TaskStatus.State.KILLED_UNCLEAN) 
			 {
				 ...
				 this.runner.start(); // run MR

				 this.taskStatus.setStartTime(System.currentTimeMillis());

				 LOG.info("Task " + task.getTaskID() + " (" + taskStatus.getTaskID().toString() + ") launched.");
			 } else {
		  ...
		  * @param TaskStatus it's the status reported by {@link Task}{@link #statusUpdate(TaskAttemptID, TaskStatus)}
		  */
		 public synchronized void reportProgress(TaskStatus taskStatus) {
			 // ReportProgress: attempt_201007151804_0001_r_000000_0_2_r_0 1.0% reduce > reduce
			 LOG.debug("ReportProgress: " + task.getTaskID() + " " + taskStatus.getProgress() + "% " + taskStatus.getStateString() 
			 ...
			 // this.taskStatus is the task status of the TaskTracker
			 if (this.done 
					 ...
					 taskStatus.getRunState() == TaskStatus.State.RUNNING)) 
			 {
				 ...
				 LOG.debug(task.getTaskID() + " Ignoring status-update since " 
						 + ((this.done) ? "task is 'done'" : ("runState: " + this.taskStatus.getRunState()))); 
				 return;
			 }
		  ...
		  * @throws IOException 
		  */
		 public synchronized void reportDone() throws IOException {
			 if (isCleaningup()) {
				 ...
				 synchronized (digestCollection) {
					 String[] digests = digestCollection.remove(getTaskID(this.taskStatus.getTaskID()));

					 if(digests != null && digests.length > 0) {
						 LOG.debug("Set digests from " + this.taskStatus.getTaskID().toString());
						 this.taskStatus.setDigests(digests);
					 }
				 }
			 }
		 ...
		 /**
		  * Report that the task has finished and release the slot
		  */
		 ...
		 private void setTaskFailState(boolean wasFailure) {
			 ...
			 {
				 taskStatus.setRunState(wasFailure ? TaskStatus.State.FAILED_UNCLEAN : TaskStatus.State.KILLED_UNCLEAN);
			 } else {
				 taskStatus.setRunState(wasFailure ? TaskStatus.State.FAILED : TaskStatus.State.KILLED);
			 }
		 }
		 ...
		 public void taskFinished() {
			 ...
			 LOG.debug("1 - Flags: " + task.getTaskID().toString() 
					 + " : done: " + done + " forcedkilled: " + forcedKilled + " waskilled: " + wasKilled);
			 //
			 ...
			 while (!done && (System.currentTimeMillis() - start < WAIT_FOR_DONE)) {
				 try {
					 ...
					 LOG.error("Taskfinished got InterruptedException");
				 }
			 }
			 ...
			 synchronized (this) {
				 ...
				 if (done || forcedKilled || !wasKilled) {
			 ...
			 LOG.debug("Task finished: " + task.getTaskID().toString() + " - " 
					 + this.taskStatus.getRunState() + " - " 
					 + this.taskStatus.getTaskID().toString()  + " (Need cleanup: " + needCleanup + " )");
		 ...
		 void cleanup(boolean needCleanup) throws IOException {
			 ...
			 synchronized (this) {
				 try {
					 ...
					 if (needCleanup) {
						 String taskDir = getLocalTaskDir(task.getJobID().toString(), taskId.toString(), task.isTaskCleanupTask());
						 if (runner != null) {
						 ...
						 if (localJobConf.getNumTasksToExecutePerJvm() == 1) {
							 directoryCleanupThread.addToQueue(defaultJobConf, getLocalFiles(defaultJobConf, taskDir));
						 }  
						 else {
							 directoryCleanupThread.addToQueue(defaultJobConf, getLocalFiles(defaultJobConf, taskDir+"/job.xml"));
						 }
					 } else {
						 if (localJobConf.getNumTasksToExecutePerJvm() == 1) {
							 String taskDir = getLocalTaskDir(task.getJobID().toString(), taskId.toString(), task.isTaskCleanupTask());
							 directoryCleanupThread.addToQueue(defaultJobConf, getLocalFiles(defaultJobConf, taskDir+"/work"));
						 }  
					 }
				 } catch (Throwable ie) {
	...
	public synchronized JvmTask getTask(JVMId jvmId) 
		...
		if (tasks.get(tip.getTask().getTaskID()) != null) { //is task still present?
			LOG.info("JVM with ID: " + jvmId + " given task: " + tip.getTask().getTaskID() + " (" + tasks.get(tip.getTask().getTaskID()).getTask() + ")");
	 ...
	 * @param TaskStatus it's the parameter sent from the Task to TaskTracker. 
	 * @see {@link Task}{@link #statusUpdate(TaskAttemptID, TaskStatus)} 
	 */
	 ...
	 * diagnostic info
	 */
	public synchronized void reportDiagnosticInfo(TaskAttemptID taskid, String info) 
			throws IOException {
		TaskInProgress tip = tasks.get(taskid);
		if (tip != null) {
	 ...
	 */
	public synchronized void commitPending(TaskAttemptID taskid, TaskStatus taskStatus) 
			throws IOException {
	...
	@Override
	public void setShuffleTime(TaskAttemptID taskid, TaskStatus taskStatus) throws IOException,
	...
	public synchronized boolean canCommit(TaskAttemptID taskid) {
		LOG.debug(taskid.toString() + " can commit? " + commitResponses.contains(taskid));

		return commitResponses.contains(taskid); //don't remove it now
	}

	/**
	 * A RT request to execute another map task
	 */
	public void requestMap(TaskAttemptID taskId) throws IOException {
		LOG.debug("Requesting another task for " + taskId.toString());
		TaskInProgress tip = runningTasks.get(taskId);
		Iterator<JobID> iter = runningJobs.keySet().iterator();
		JobID res = null;
		while(iter.hasNext()) {
			JobID jid = iter.next();
			RunningJob rjob = runningJobs.get(jid);
			if(rjob.tasksSet.contains(tip)) {
				res = jid;
				break;
			}
		}

		jobClient.requestMap(res, taskId);
	}
	...
	public void sendDigest(TaskAttemptID taskid, String[] digest) 
		...
		synchronized (digestCollection) {
			...
			digestCollection.put(getTaskID(taskid), digest);
		}
	}
	...
	public synchronized void done(TaskAttemptID taskid) 
		...
		if(commitResponses.contains(taskid))
			LOG.debug("Removing " + taskid + " from commitResponses");

		commitResponses.remove(taskid);
		if (tip != null) {
			LOG.debug(tip.getTask().getTaskID().toString() + " is reporting done.");
			tip.reportDone();
		} else {
			LOG.warn("Unknown child task done: "+taskid.toString() +". Ignored.");
		}
	}
	...
	/**
	 * Get all MapCompletionEvents 
	 * @param fromEventId is an offset to retrieve the list of tasks completed.
	 */
	public synchronized MapTaskCompletionEventsUpdate getMapCompletionEvents(JobID jobId, int fromEventId, int maxLocs, TaskAttemptID id) 
			throws IOException {
		TaskCompletionEvent[] mapEvents = TaskCompletionEvent.EMPTY_ARRAY;
	...
	void reportTaskFinished(TaskAttemptID taskid, boolean commitPending) {
		...
		LOG.info("TaskFinished: " + taskid.toString() + " CommitPending - " + commitPending);
	...
	public synchronized void mapOutputLost(TaskAttemptID taskid, String errorMsg) 
		...
		LOG.error("Map output has lost for " + taskid);
	...
	static class RunningJob{
		...
		volatile Set<TaskInProgress> tasksSet;
		boolean localized;
		boolean keepJobFiles;
		FetchStatus f;
		RunningJob(JobID jobid, boolean keep) {
			this.jobid = jobid;
			localized = false;
			tasksSet = new HashSet<TaskInProgress>();
			keepJobFiles = keep;
		}
	 ...
	 * @return a copy of the list of TaskStatus objects
	 */
	List<TaskStatus> taskFiles = new ArrayList<TaskStatus>();
	synchronized List<TaskStatus> getRunningTaskFiles() {
		for(TaskInProgress tip: runningTasks.values()) {
			if(tip.getTask().isMapTask() && !contains(taskFiles, tip))
				taskFiles.add(tip.getStatus());
		}

		return taskFiles;
	}

	private boolean contains(List<TaskStatus> status, TaskInProgress tip) {
		for(TaskStatus st : status) {
			if(st.getTaskID().equals(tip.getTask().getTaskID()))
				return true;
		}
	...
	public static Map<Integer, List<InfoOutput>> iout = Collections.synchronizedMap(new HashMap<Integer, List<InfoOutput>>());

	public static class InfoOutputServlet extends HttpServlet {
		public void doGet(HttpServletRequest request, HttpServletResponse response)
				throws ServletException, IOException {
			StringBuffer result = new StringBuffer();

			synchronized (iout) {
				Iterator iter = iout.keySet().iterator();

				while(iter.hasNext()) {
					int key = (Integer) iter.next();

					List<InfoOutput> list = iout.get(key);

					for(InfoOutput iot : list) {
						result.append("<tr><td>" + iot.getReduceTask() 
								+ "</td><td>" + iot.getMapId() 
								+ "</td></tr>");
					}
				}
	...
	public static Map<Integer, List<InfoOutput>> mapout = new HashMap<Integer, List<InfoOutput>>();

	/**
	 * This class is used in TaskTracker's Jetty to serve the map outputs
	 * to other nodes.
	 * 
	 * ReduceTask requests TaskTracker's map output data
	 */
	public static class MapOutputServlet extends HttpServlet {
		...
		public void doGet(HttpServletRequest request, HttpServletResponse response)
				throws ServletException, IOException {
			// example of request:
				//http://hawai:50060/mapOutput?job=job_201002141740_0001&map=attempt_201002141740_0001_m_000004_0_0&part=0
			// or http://virtuakarmic:50060/mapOutput?job=job_201004191916_0001&map=attempt_201004191916_0001_m_000005_1_2_m_0&reduce=1&reduceTask=attempt_201004191916_0001_r_000001_0_0_r_0
			...
			TaskAttemptID mapTaskId 	= TaskAttemptID.forName(mapId);
			TaskAttemptID reduceTaskId  = TaskAttemptID.forName(reduce);
			int reduceReplicaNr = getReplicaNumber(reduceTaskId.getTaskID());
			...
			try {
				...
				Path indexFileName = lDirAlloc.getLocalPathToRead(
						...
						+ "/" + MapOutputFile.getFileOutIndex(), conf);

				// Map-output file
				// For example:
				// /tmp/dir/hadoop-hadoop/mapred/local/taskTracker/jobcache
				// /job_201002141402_0001/attempt_201002141402_0001_m_000000_0_0/output/file.out
				Path mapOutputFileName = lDirAlloc.getLocalPathToRead(TaskTracker.getIntermediateOutputDir(jobId, mapId) 
						+ "/" + MapOutputFile.getFileOut(), conf);

				LOG.debug("MapOutputServlet: <" + part + "> wants " + mapId 
						+ ". IndexFileName: " + indexFileName + ", " + mapOutputFileName);
				int mapReplicaNr = getReplicaNumber(mapTaskId.getTaskID());
				int idx = (mapTaskId.getTaskID().getId() * conf.getFaultTolerance()) + mapReplicaNr;

				synchronized (iout) {
					if(!iout.containsKey(idx))
						iout.put(idx, new ArrayList<InfoOutput>());
				}

				FileStatus status = rfs.getFileStatus(mapOutputFileName);
				iout.get(idx).add(new InfoOutput(mapId, reduce, jobId, ""+status.getLen()));
				...
				IndexRecord info = tracker.indexCache.getIndexInformation(mapId, partition, indexFileName);
				...
				response.setHeader(FOR_REDUCE_TASK, Integer.toString(partition));

				response.setHeader(START_OFFSET, Long.toString(info.startOffset));
				...
				FileStatus st = rfs.getFileStatus(mapOutputFileName);
				long length = st.getLen();

				try {
					mapOutputIn = rfs.open(mapOutputFileName);
				} catch(IOException e) {
					LOG.error("Map output file not found: " + mapOutputFileName);
					throw e;
				}

				if(LOG.isDebugEnabled()) {
					LOG.debug("[" + reduceTaskId.toString() + "] " + mapTaskId.toString() + " - length: " + length 
							+ " - offset: " + info.startOffset 
							+ " - partlength: " + info.partLength);
				}

				//seek to the correct offset for the reduce
				mapOutputIn.seek(Long.valueOf(info.startOffset)); 
				long rem = Long.valueOf(info.partLength); 
				int len  = mapOutputIn.read(buffer, 0, (int)Math.min(rem, MAX_BYTES_TO_READ));

				if(LOG.isDebugEnabled()) {
					LOG.debug("[" + reduceTaskId.toString() + "] read " + len + " [" + mapTaskId.toString() + "]"); 
				}
				...
				while (rem > 0 && len >= 0) {
					...
					try {
						...
						// the exception thrown here is harmless
						// see https://issues.apache.org/jira/browse/MAPREDUCE-5
					...
					if(LOG.isDebugEnabled()) {
						LOG.debug("[" + reduceTaskId.toString() + "] totalRead " + totalRead + " remain: " + rem
								+ " [" + mapTaskId.toString() + "]"); 
					}
					...
					if(LOG.isDebugEnabled()) {
						LOG.debug("[" + reduceTaskId.toString() + "] read " + len + " remain: " + rem 
								+ " [" + mapTaskId.toString() + "]"); 
					}
				}

				LOG.info("MapOutputServlet sent out " + totalRead + " bytes for reduce: " + partition + 
						" from map: " + mapId + " given " + info.partLength + "/" + info.rawLength);
			} catch (IOException ie) {
				// some time errors from this aren't important
				Log log = (Log) context.getAttribute("log");
				String errorMsg = ("getMapOutput(" + mapId + "," + part + ") failed :\n"+ StringUtils.stringifyException(ie));
				log.warn(errorMsg);
	...
	public boolean shouldTamper(TaskAttemptID taskid, int choice) throws IOException {
		if(choice == 1)
			return jobClient.shouldTamperMapDigest(taskid);
		else if(choice == 2)
			return jobClient.shouldTamperMapFile(taskid);
		else if(choice == 3)
			return jobClient.shouldTamperReduceDigest(taskid);

		return false;
	}

	public void executeAnother(TaskAttemptID reduceTask) {
		synchronized (tentativeExecutionTasks) {
			tentativeExecutionTasks.add(reduceTask);
		}
	}

	private static class FetchSet {
		...
		public FetchSet(String reduceId, String mapId) {
		...
		public int hashCode() {
			...
			result = prime * result
					+ ((reduceId == null) ? 0 : reduceId.hashCode());
			return result;
		}
		...
		public boolean equals(Object obj) {
			...
			if (mapId == null) {
				...
				return false;
			if (reduceId == null) {
				if (other.reduceId != null)
					return false;
			} else if (!reduceId.equals(other.reduceId))
				return false;
			return true;
		}
	}
