...
class CleanupQueue {
	...
	private static class PathCleanupThread extends Thread {
		...
		public void run() {
			LOG.debug("CleanupQueue: " + getName() + " started.");
			PathAndConf pathAndConf = null;
			while (true) {
				try {
					pathAndConf = queue.take();

					if(!pathAndConf.conf.getKeepJobFiles()) {
						// delete the path.
						FileSystem fs = pathAndConf.path.getFileSystem(pathAndConf.conf);
						fs.delete(pathAndConf.path, true);

						LOG.debug("CleanUpQueue DELETED " + pathAndConf.path);
					} else {
						// /tmp/taskTracker/jobcache/job_201105131941_0006/attempt_201105131941_0006_7155_m_000005_1_0/work
						// /tmp/taskTracker/jobcache/job_201105131941_0006/attempt_201105131941_0006_13477_r_000005_1_0
						LOG.info("Won't clean " + pathAndConf.path);
					}
