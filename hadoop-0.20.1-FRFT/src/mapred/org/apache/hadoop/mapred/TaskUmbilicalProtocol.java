...
interface TaskUmbilicalProtocol extends VersionedProtocol {
    ...
    /**
     * Request to execute another task
     * @param taskid
     * @throws IOException
     */
    ...
    void setShuffleTime(TaskAttemptID taskid, TaskStatus taskStatus)
            throws IOException, InterruptedException; 
    ...
    /**
     * Get map tasks that are forced killed
     * @param taskAttemptId
     */
    //	TaskAttemptID[] getTaskForcedKilled(TaskAttemptID taskAttemptId);


    void executeAnother(TaskAttemptID reduceTask) throws IOException;
    
    /**
     * Asks the TT if should tamper the data
     * @return
     * @throws IOException
     */
    boolean shouldTamper(TaskAttemptID taskid, int choice) throws IOException;
}
